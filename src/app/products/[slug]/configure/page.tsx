...
function ConfigurePageContent() {
  ...
  const generateCode = () => {
    ...
    if (config.requests.appAccount?.enabled) {
      ...
      lines.push(`    defaultAccount: '${defaultAccount}', // ${defaultAccount === 'sub' ? 'Use app account as default' : 'Use main account as default'}`);
      lines.push(`    funding: '${funding}', // ${funding === 'spend-permissions' ? 'Route through main account if needed' : 'Direct execution from app account'}`);
      lines.push('  },');
    }
    ...
    const hasCapabilities = config.capabilities.signInWithEthereum ||
    ...
    if (hasCapabilities) {
      ...
      if (config.requests.spendPermission?.enabled) {
        ...
        lines.push('            {');
        lines.push('              token: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", // USDC on Base');
        const allowanceInWei = parseFloat(config.requests.spendPermission.allowance) * 1e6; // USDC has 6 decimals
        lines.push(`              allowance: "0x${allowanceInWei.toString(16)}", // ${config.requests.spendPermission.allowance} USDC`);
        const periodMap = { Daily: 86400, Weekly: 604800, Monthly: 2592000 };
        const period = periodMap[config.requests.spendPermission.frequency as keyof typeof periodMap] || 86400;
        lines.push(`              period: ${period}, // ${config.requests.spendPermission.frequency}`);
        lines.push('            },');
        lines.push('          ],');
        lines.push('        },');
      }
      ...
      if (config.requests.appAccount?.enabled) {
        ...
        lines.push('      // during wallet_connect because we set creation: \'on-connect\' in SDK config');
      }

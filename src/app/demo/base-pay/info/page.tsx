...
export default function BasePayInfoPage() {
  ...
  const handleContinue = () => {
    router.push(
      `/demo/base-pay/confirm?config=${encodeURIComponent(JSON.stringify(config))}&viewMode=${viewMode}`
    );
  };
  ...
  const footer = (
    <div className={`flex ${spacing.gap.md}`}>
      ...
      <Button
        ...
        className="flex-1"
      >
        {!isScrolledToBottom && requiredRequests.length + optionalRequests.length > 3
          ? 'Scroll to continue'
          : 'Continue'}
      </Button>
    </div>
          ...
          <div className={spacing.section.lg}>
            ...
            <div className="space-y-3">
              {requiredRequests.map((request) => (
                ...
                >
                  <div className="text-gray-700">{getFieldIcon(request.type)}</div>
